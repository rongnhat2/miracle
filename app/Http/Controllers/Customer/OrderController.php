...
class OrderController extends Controller
{
    ...
    public function __construct(Product $product, Customer $customer, FullOrder $order, OrderDetail $order_detail, Discount $discount){
    ...
    public function checkout(Request $request){
        ...
        foreach ($item as $key => $item_id) {
            ...
            if ($product) { 
                ...
                $discount_value_     += $quantity[$key] * $discount_product;
                $data_sub_order = [
                    "order_id"      => 0,
                    "product_id"    => $item_id,
        ...
        $total = $sub_total - $discount_value_;

        $data_order = [
            "customer_id"       => $user_id ?? 0,
            "sub_total"         => $sub_total,
            "discount_total"    => $discount_value_,
            "total"             => $total,
            "username"          => $name,
            "email"             => $email,
            "telephone"         => $phone,
        ...
        if ($payment == 1) {
            ...
            return $this->order->send_response("Order Successful, redirect VNPay", $route_redirect, 200); 
        }
    }
